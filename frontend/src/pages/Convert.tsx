...
export function Convert() {
  ...
  const toAmount =
    fromAmount && exchangeRate > 0 ? (Number(fromAmount) * exchangeRate).toFixed(6) : '';
  const usdValue =
    fromAmount && fromUSD > 0
      ? (Number(fromAmount) * fromUSD).toLocaleString('en-US', {
          style: 'currency',
          currency: 'USD',
        })
      : '';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (fromToken === toToken) {
      setError('Select different tokens to convert');
      return;
    }
  ...
  const tokenSelect: CSSProperties = {
    background: '#162040',
    ...
    <div style={container}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={card}>
            ...
            >
              ...
              <p
                style={{
                  fontSize: '24px',
                  fontWeight: '700',
                  color: toAmount ? '#34d399' : 'var(--text-muted)',
                  fontVariantNumeric: 'tabular-nums',
                }}
              >
                {toAmount || '0.00'}
          ...
          <div
            style={{
              background: 'rgba(96,128,245,0.06)',
              border: '1px solid rgba(96,128,245,0.15)',
              borderRadius: '10px',
              padding: '12px 14px',
              marginBottom: '20px',
            }}
            ...
            {loadingPrices ? (
              ...
              >
                <span style={{ color: 'var(--text-muted)', fontSize: '13px' }}>
                  1 {fromToken} ≈{' '}
                  {exchangeRate > 0
                    ? exchangeRate.toFixed(fromToken.startsWith('USD') ? 6 : 4)
                    : '—'}{' '}
                  {toToken}
                </span>
                <span style={{ color: 'var(--text-muted)', fontSize: '12px' }}>
                  {fromToken}: ${prices[fromToken]?.toLocaleString() || '—'} · {toToken}: $
                  {prices[toToken]?.toLocaleString() || '—'}
                </span>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            <button
              ...
              style={{
                flex: 2,
                padding: '14px',
                background: submitting || !fromAmount ? '#1e2a40' : 'var(--primary)',
                border: 'none',
                borderRadius: '12px',
                color: submitting || !fromAmount ? 'var(--text-muted)' : 'white',
                cursor: submitting || !fromAmount ? 'not-allowed' : 'pointer',
                fontWeight: '700',
                fontSize: '15px',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                gap: '8px',
                boxShadow: fromAmount && !submitting ? 'var(--shadow-primary)' : 'none',
                transition: 'all 0.2s',
              }}
            >
              {submitting ? (
      ...
      >
        Rates update every 30 seconds from CoinGecko. Actual execution price may vary slightly due
        to network conditions and slippage. Max slippage: 1%.
      </p>
    </div>
  );
}
