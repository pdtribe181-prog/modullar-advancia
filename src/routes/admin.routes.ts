 ...
 */
router.get('/transactions/:id', validateParams(idParamsSchema), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  ...
  if (transaction.stripe_payment_intent_id) {
    try {
      ...
      logger.warn('Failed to fetch Stripe payment', { paymentIntentId: transaction.stripe_payment_intent_id });
    }
  }
 ...
 * GET /admin/disputes
 */
router.get('/disputes', validateQuery(disputeQuerySchema), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
 ...
 */
router.patch('/disputes/:id', validateParams(idParamsSchema), validateBody(updateDisputeSchema), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
 ...
 */
router.get('/providers/:id/stripe', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
 ...
 * GET /admin/audit-log
 */
router.get('/audit-log', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
 ...
 */
router.get('/analytics/revenue', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
...
router.get('/system/health', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const allHealthy = Object.values(checks).every((c) => (c as { status: string }).status === 'healthy');
