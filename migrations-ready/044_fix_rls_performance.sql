-- Migration 044: Fix RLS performance by wrapping auth.uid() in a select statement
...
CREATE POLICY "Users can view and providers can manage insurance claims" ON public.insurance_claims
    FOR ALL
    USING (
        patient_id IN (SELECT id FROM public.patients WHERE user_id = (select auth.uid())) OR
        provider_id IN (SELECT id FROM public.providers WHERE user_id = (select auth.uid())) OR
        EXISTS (
            SELECT 1 FROM public.user_profiles
            WHERE user_profiles.id = (select auth.uid())
            AND user_profiles.role IN ('admin', 'staff')
...
CREATE POLICY "Users can view own payment plan transactions and admins can manage all" ON public.payment_plan_transactions
    FOR ALL
    USING (
        payment_plan_id IN (SELECT id FROM public.payment_plans WHERE patient_id = (select auth.uid())) OR
        EXISTS (
            SELECT 1 FROM public.user_profiles
            WHERE user_profiles.id = (select auth.uid())
            AND user_profiles.role = 'admin'
