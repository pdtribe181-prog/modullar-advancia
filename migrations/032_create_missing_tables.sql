...
-- ============================================================
-- Creates 20 tables that are referenced in the codebase but missing
-- from the database. All use IF NOT EXISTS for idempotency.
-- RLS policies use SECURITY DEFINER helpers from migration 031.
-- ============================================================
...
-- These use uuid_generate_v4() and reference auth.users
-- ============================================================

-- 1. compliance_status (from 006)
...
-- 2. compliance_violations (from 006)
...
-- 4. provider_performance_metrics (from 007)
...
-- 9. crypto_transactions (from 019)
...
-- Uses SECURITY DEFINER helpers (no direct subquery on patients/providers)
...
CREATE POLICY "providers_view_own_crypto" ON public.crypto_transactions
  FOR SELECT TO authenticated
  USING (provider_id = public.get_provider_id());
...
-- 11. wallet_transactions (from 021)
...
-- Uses SECURITY DEFINER helper (no direct subquery on providers)
...
-- 12. wallet_verification_challenges (from 021)
...
-- Uses is_admin() SECURITY DEFINER (no direct subquery on user_profiles)
...
-- 15. notification_queue (from 023)
...
-- Uses is_admin() and is_provider() — no self-referencing subqueries
...
-- Uses SECURITY DEFINER helpers (no direct subqueries on patients/user_profiles)
...
-- 18. payment_plan_transactions (from 024)
...
-- Uses SECURITY DEFINER helpers (no direct subqueries)
